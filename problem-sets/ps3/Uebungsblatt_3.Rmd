...
```{r "1_a"}
eu_makro <- read_csv("C:/Users/Jonas/OneDrive/Dokumente/GitHub/25-26/problem-sets/ps3/data/EU_Makro.csv")
...
g2<- ggplot(deutschland) +
  geom_point(aes(x = year, y = gdp), color = "steelblue") +
  geom_line(aes(x = year, y = gov_debt), color ="firebrick") +
  labs( 
    title = "Bruttoinlandsprodukt und Schulden Deutschlands über die Jahre",
    x = "Jahr",
    y = "Wert (in Mrd. EUR)",
    color = "Variable"
    ) +
...
g4 <- eu_analyse |>
  ggplot() + 
  geom_line(aes(x = year, y = debt_share_country, group = Laender, colour = Laender))
...
g5 <- g4 + 
  ylab("Schuldenquote") + 
  xlab("Jahr") +
  scale_colour_manual(name = "Andere", 
